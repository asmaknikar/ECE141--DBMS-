...
namespace ECE141 {
  ...
  static std::vector<ECE141::Keywords> gJoinTypes={
  ...
  public:
    ...
    static DataTypes keywordToDatatype(Keywords aKeyword) {
        switch (aKeyword) {
        case Keywords::char_kw: return DataTypes::varchar_type;
    ...
    static std::string indToHexChars(ind aNumber) {
        char str[9]= {0};
        std::to_chars(std::begin(str), std::end(str), aNumber, 16);
        str[8] = '\0';
        if (std::string{ str } == "") {
            return "";
        }
    ...
    static ind hexCharsToInd(std::string aString) {
        if (aString.length() == 0 || !std::isxdigit(aString[0])) return 0;
        int theResult = std::stoi(aString, 0, 16);
        return static_cast<ind>(theResult);
    }
    static ind hash(const std::string str) {
        ind theResult = 0;
        for (size_t i = 0; i < str.length(); i++)
        {
            theResult += (i * str[i]) % std::numeric_limits<ind>::max();
        }
        return theResult;
    }
    template<typename T>
    static bool in_array(std::vector<T> aList, T anItem) {
        for (size_t i = 0; i < aList.size(); i++) {
            if (aList[i] == anItem) return true;
        }
        return false;
    }
